...
В этом проекте q агент обучается проходить лабиринт на основе координат где он находится, важно учитывать что нейросети в этоп проекте нет, только q агент
...
```
## Примечания
В проект прикреплены данные после моего обучения, они сохранены как brain.json. Можете удалить этот файл и обучить своего агента. Так же, когда ваш агент достаточно обучился, вы можете выключить программу и зайти в файл q_agent.py и изменить следующий код:
...
1. with open() для того чтобы загрузить результат вашего агента
2. epsilon должен быть 0 для того чтобы агент не использовал "жадную" стратегию обучения, когда агент обучился жадная стратегия будет только мешать, так как из-за неё агент будет выполнять случайные действия
